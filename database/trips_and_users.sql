...
-- Users table. Status is an ENUM type of (‘completed’, ‘cancelled_by_driver’,
-- ‘cancelled_by_client’).
--
--     +----+-----------+-----------+---------+--------------------+----------+
...
-- query should return the following rows with the cancellation rate being
...
-- Credits:
--
--     Special thanks to @cak1erlizhou for contributing this question, writing
--     the problem description and adding part of the test cases.


  SELECT Request_at, ROUND(SUM(IF(Status LIKE 'cancelled%', 1, 0)) / SUM(1), 2)
    FROM (SELECT Request_at, Status
            FROM Trips
